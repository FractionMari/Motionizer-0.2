// Denne versjonen er fra 22. oktober 2020
...
var DiffCamEngine = (function() {
	...
	function init(options) {
		...
		diffWidth = options.diffWidth || 13;
		diffHeight = options.diffHeight || 2;
	...
	function processDiff(diffImageData) {
		...
		for (var i = 0; i < rgba.length; i += 4) {
			...
			if (pixelDiff >= pixelDiffThreshold) {
				...
				var x_playing = coords.x;

                var y_playing = coords.y;
                var gainScore = score / 100;

				var o = ctx.createOscillator();
				o.type = 'saw';
                gainNode.gain.setValueAtTime(gainScore, ctx.currentTime);
                o.connect(gainNode);
                gainNode.connect(ctx.destination);
				o.frequency.value = x_playing * 150;
